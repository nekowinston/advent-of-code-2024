...
parseInstructions acc input =
  case input =~ mulRegex :: RegexMatches of
    (_, _, rest, [x, y]) -> parseInstructions ((unwrapDecimal x, unwrapDecimal y) : acc) rest
    _ -> reverse acc
  ...
  case input =~ mulRegex :: RegexMatches of
    (prefix, _, rest, [x, y]) -> parseInstructions' ((x', y') : acc) shouldAct rest
     where
      ...
      (x', y') = if shouldAct then (unwrapDecimal x, unwrapDecimal y) else (0, 0) -- replace it with a noop if disabled
    _ -> reverse acc
